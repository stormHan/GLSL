...
void RenderPass()
{
	...
	pLightShader->SetgWorld(p.GetWorldTrans());
	p.SetCamera(m_SpotLight.getLightPosition(), m_SpotLight.GetSpotLightDir(), Vector3f(0.0f, 1.0f, 0.0f));
	pLightShader->SetPointLights(1, &m_SpotLight);
	pLightShader->SetLightWVP(p.GetWVPTrans());
...
static void RenderSceneCB()
{
	...
	*/
	...
	glPointSize(10.0f);
	glBegin(GL_POINTS);
		glVertex3f(m_SpotLight.getLightPosition().x, m_SpotLight.getLightPosition().y, m_SpotLight.getLightPosition().z);
	glEnd();
	glPopMatrix();
