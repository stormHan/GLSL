...
GLuint Texture::loadBMP_custom(const char * imagepath){
	...
	glGenTextures(1, &m_textrueObj);

	// "Bind" the newly created texture : all future texture functions will modify this texture
	glBindTexture(m_textureTarget, m_textrueObj);
	...
	glBindTexture(m_textureTarget, 0);//release the tex first.
...
GLuint Texture::loadDDS(const char * imagepath){
	...
	for (unsigned int level = 0; level < mipMapCount && (width || height); ++level)
	{
		...
		glCompressedTexImage2D(m_textureTarget, level, format, width, height,
