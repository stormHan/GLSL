...
void Object::loadObjfromFile(char *filename)
{
	this->path = filename;
	if (!pMode)
	{
		pMode = glmReadOBJ(path);
	}
	if (!pMode)//是否有模型加载进来
	{
...
{
	glmUnitize(pMode);//单位化
	glmScale(pMode, 0.5);
	glmFacetNormals(pMode);
	glmVertexNormals(pMode, 90.0);
}

void Object::LoadTextureViaBMP(Texture tex, char *filename)
{

	tex.loadBMPTexture(tex.textureBMP, filename);
}

void Object::LoadTextureViaTGA(Texture tex, char *filename)
{
	tex.loadTGATexture(filename);
}
...
glmReverseWinding(GLMmodel* model)
{
	...
	/* reverse facet normals */ //面法向量从1开始计数
...
glmReadOBJ(char* filename)
{
	...
	of vertices, normals, texcoords & triangles */ //得到多面体的顶点  ， 纹理 ， 数量信息
